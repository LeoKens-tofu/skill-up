...
export default function TrashPage() {
  ...
  const fetchTrash = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:4000/api/client/teacher/quizzes/trash", {
        credentials: "include"
      });
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/client/teacher/quizzes/restore/${id}`, {
        method: "PATCH",
        credentials: "include"
      });
      const json = await res.json();
  ...
  const handleHardDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/client/teacher/quizzes/hard/${id}`, {
        method: "DELETE",
        credentials: "include"
      });
      const json = await res.json();
