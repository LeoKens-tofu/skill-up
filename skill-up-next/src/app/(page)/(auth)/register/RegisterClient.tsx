...
export default function RegisterClient() {
  ...
  useEffect(() => {
    ...
    if (activeTab === "student") {
      validatorRef.current.addField('#studentId', [
        { rule: 'required', errorMessage: 'Mã sinh viên không được để trống' },
      ]);
    }
    ...
    validatorRef.current
      .addField('#password', [
        ...
        { rule: 'minLength', value: 6, errorMessage: 'Mật khẩu phải từ 6 ký tự' },
      ])
      .addField('#confirmPassword', [
        { rule: 'required', errorMessage: 'Xác nhận mật khẩu không được để trống' },
        {
      ...
      .onSuccess(async (event: any) => {
        ...
        const studentId = activeTab === "student" ? (document.getElementById('studentId') as HTMLInputElement).value : undefined;
        const password = (document.getElementById('password') as HTMLInputElement).value;

        try {
          setIsLoading(true);
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/auth/register`, {
            method: "POST",
            headers: {
  ...
  const theme = THEME;
  const placeholder =
    activeTab === "student" ? "VD: nguyenvana@fpt.edu.vn" : "VD: giang.vien@fpt.edu.vn";
  const idLabel =
    activeTab === "student" ? "Email sinh viên" : "Email giảng viên";
  ...
  return (
    ...
    >
      ...
      <div className="w-[45%] flex items-center justify-center px-16 py-10 transition-colors duration-500 overflow-y-auto">
